...
def _add_bullet_slide(slide, items, start_top, left=Inches(0.8), width=Inches(11.5),
    ...
    Each item is a 2-tuple (title, desc) or 3-tuple (title, desc, url).
    When *url* is provided the title becomes a clickable hyperlink.
    """
...
def _style_cell(cell, text, bold=False, bg=None, fg=WHITE, size=13, hyperlink=None):
    cell.text = ""
...
items = [
    ...
    ("Pantone \u2013 Agentic AI Case Study (Feb 12, 2026)",
     ...
     "https://azure.microsoft.com/en-us/blog/the-data-behind-the-design-how-pantone-built-agentic-ai-with-an-ai-ready-database/"),
...
data = [
    ...
    ["High-Temp Superconductors", "Infrastructure", "Exploring superconductors for datacenter power", "Feb 10",
     "https://azure.microsoft.com/en-us/blog/can-high-temperature-superconductors-transform-the-power-infrastructure-of-datacenters/"],
...
for r, row_data in enumerate(data):
    bg = row_bg[r % 2]
    vals = row_data[:4]
    url = row_data[4] if len(row_data) > 4 else None
    for c, val in enumerate(vals):
        link = url if c == 0 else None
        _style_cell(table.cell(r + 1, c), val, bg=bg, fg=WHITE, hyperlink=link)
...
for name, url in links:
    _add_text_box(slide, Inches(1.0), y, Inches(4), Inches(0.4), name,
                  font_size=16, bold=True, color=ACCENT, hyperlink=url)
    _add_text_box(slide, Inches(5.2), y, Inches(7.5), Inches(0.4), url,
                  font_size=14, color=LIGHT_GRAY, hyperlink=url)
    y += Inches(0.55)
